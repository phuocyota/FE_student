...
//       try {
//         // 🔥 1. Lấy danh sách examSet theo subject
//         const res = await fetch(API.EXAM_SET.GET_BY_SUBJECT(subjectId));
//         const payload = await parseResponse(res);

//         const examSets = payload?.data?.data || [];

//         // 🔥 2. Gọi API detail từng examSet
//         const requests = examSets.map((examSet) =>
//           fetch(API.EXAM_SET.DETAIL(examSet.id))
//             .then(parseResponse)
//             .then((res) => ({
//               examSetName: res?.data?.name,
//               questionBanks: res?.data?.questionBanks || [],
//             }))
//         );

//         const results = await Promise.all(requests);

//         // 🔥 3. Gộp tất cả questionBanks
//         let allQuestions = [];

//         results.forEach((item) => {
//           const mapped = item.questionBanks.map((q) => ({
//             id: q.id,
//             title: q.title,
...
//             type: item.examSetName, // hiển thị tên bộ đề
//           }));

//           allQuestions = [...allQuestions, ...mapped];
//         });

//         setExamData(allQuestions);
//       } catch (error) {
//         console.error(error);
//       } finally {
//         setLoading(false);
